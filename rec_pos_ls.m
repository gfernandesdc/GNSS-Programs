function [pos, El, GDOP, basic_obs] = rec_pos_ls(svpdistO,sow,Eph)
...
%$Revision: 1.1 $  $Date: 2002/07/10  $
    ...
    v_light = 299792458;
    dtr     = pi/180;
    m       = size(svpdistO,1);  % number of svs
    %el      = zeros(m,1);
%    
...
% preliminary guess for receiver position and receiver clock offset
    ...
    for iter = 1:no_iterations
        ...
        for i = 1:m        
%         k = sats(i);
            tx_RAW = sow - svpdistO(i)/v_light;
            t0c = Eph(20,i);
            dt = check_t(tx_RAW-t0c);
            tcorr = (Eph(1,i)*dt + Eph(19,i))*dt + Eph(18,i);
            tx_GPS = tx_RAW-tcorr;
            dt = check_t(tx_GPS-t0c);
            tcorr = (Eph(1,i)*dt + Eph(19,i))*dt + Eph(18,i);
            tx_GPS = tx_RAW-tcorr;
            X = satpos(tx_GPS, Eph(:,i));
            if iter == 1
                traveltime = 0.072;
                Rot_X = X;
                trop = 0;
 ...
 % v_light*tcorr is the satellite clock offset
            if iter == no_iterations
                ps_corr = [ps_corr; svpdistO(i)+v_light*tcorr-trop];
                sat_pos = [sat_pos; X'];
            end
            omc = [omc; svpdistO(i)-norm(Rot_X-pos(1:3),'fro')-pos(4)+v_light*tcorr-trop]; 
            A = [A; (-(Rot_X(1)-pos(1)))/svpdistO(i)...
                (-(Rot_X(2)-pos(2)))/svpdistO(i) ...
                (-(Rot_X(3)-pos(3)))/svpdistO(i) 1];
        end % i
        x = A\omc;
        pos = pos+x;
